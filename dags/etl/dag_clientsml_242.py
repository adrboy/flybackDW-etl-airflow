# ═══════════════════════════════════════════════════════
# DAG: dag_clientsml_242
# Objetivo: ETL clientes ML desde MariaDB 242 → SQL Server
# Carpeta: etl/
...
# ── Configuración ────────────────────────────────────────
VISTA_ORIGEN  = "db_general.viewclientsml"
TABLA_DESTINO = "source.clientsml"
SQL_SELECT    = "sql/clients/select_clientsml_242.sql"
SQL_INSERT    = "sql/clients/insert_clientsml_242.sql"
...
def etl_clientsml():
    ...
    try:
        ...
        mensaje_log.append(f"[DAG: dag_clientsml_242] MAX clientid destino: {max_id}")

        # Paso 2 — ETL con SQL externo + executemany
        filas = ejecutar_insert(
            dag_id          = "dag_clientsml_242"
        ...
        mensaje_log.append(f"[DAG: dag_clientsml_242] Filas insertadas: {filas}")
        estado = "SUCCESS"

    except Exception as e:
        mensaje_log.append(f"[DAG: dag_clientsml_242] ERROR: {str(e)}")
        mensaje_error = str(e)
        raise
        ...
        try:
            ...
            escribir_log_txt(LOG_PATH, "clientsml", "\n".join(mensaje_log))
...
with DAG(
  ...
  , catchup           = False
  , tags              = ["bronze", "242", "clientsml"]
) as dag:

    tarea_etl = PythonOperator(
        task_id         = "etl_clientsml"
      , python_callable = etl_clientsml
      , retries         = 3
      , retry_delay     = 60
    )
