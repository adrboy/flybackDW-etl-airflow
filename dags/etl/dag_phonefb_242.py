# ═══════════════════════════════════════════════════════
# DAG: dag_phonefb_242
# Objetivo: ETL phones FB desde MariaDB 242 → SQL Server
# Carpeta: etl/
...
def etl_phonefb_242():
    ...
    try:
        filas = ejecutar_truncate_insert(
            dag_id          = "dag_phonefb_242"
          , mariadb_conn_id = ORIGEN_CONN_ID_242
          , mssql_conn_id   = MSSQL_CONN_ID
          , vista_origen    = VISTA_ORIGEN
          , tabla_destino   = TABLA_DESTINO
        )
        mensaje_log.append(f"[DAG: dag_phonefb_242] TRUNCATE + INSERT exitoso")
        mensaje_log.append(f"[DAG: dag_phonefb_242] Filas insertadas: {filas}")
        estado = "SUCCESS"

    except Exception as e:
        mensaje_log.append(f"[DAG: dag_phonefb_242] ERROR: {str(e)}")
        mensaje_error = str(e)
        raise
        ...
        try:
            ...
            escribir_log_txt(LOG_PATH, "phonefb", "\n".join(mensaje_log))
...
with DAG(
  ...
  , catchup           = False
  , tags              = ["bronze", "242", "phonefb"]
) as dag:

    tarea_etl = PythonOperator(
        task_id         = "etl_phonefb_242"
      , python_callable = etl_phonefb_242
      , retries         = 3
      , retry_delay     = 60
    )
