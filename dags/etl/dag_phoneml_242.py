# ═══════════════════════════════════════════════════════
# DAG: dag_phoneml_242
# Objetivo: ETL phones ML desde MariaDB 242 → SQL Server
# Carpeta: etl/
...
def etl_phoneml_242():
    ...
    try:
        ...
        mensaje_log.append(f"[DAG: dag_phoneml_242] TRUNCATE + INSERT exitoso")
        mensaje_log.append(f"[DAG: dag_phoneml_242] Filas insertadas: {filas}")
        estado = "SUCCESS"

    except Exception as e:
        mensaje_log.append(f"[DAG: dag_phoneml_242] ERROR: {str(e)}")
        mensaje_error = str(e)
        raise
